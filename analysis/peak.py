...
def pick_peaks(
    ...
    for key, data_points in dp_dict.items():
        if key not in vial_names:
            # NOTE: actually it shd not happens since both vial_names and
            # dp_dict are generated automatically from the same txt files,
            # but I still add this clause to hint unexpected error
            continue
        ...
        for dp in data_points:
            for a in analytes:
                ...
                upper_tolerance = rt[a] + upper_threshold
                lower_tolerance = rt[a] - lower_threshold
